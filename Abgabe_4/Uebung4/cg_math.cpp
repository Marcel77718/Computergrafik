...
double crop(double x, double l, double u) noexcept
{
	return std::min(u,std::max(l,x));
}

// crop all componentens of x to interval [l,u]
CVec3d crop(const CVec3d& x, double l, double u) noexcept
{
	return CVec3d(crop(x[0], l, u), crop(x[1], l, u), crop(x[2], l, u));
}

// crop all componentens of x to interval [l,u]
Color crop(const Color& x, double l, double u) noexcept
{
